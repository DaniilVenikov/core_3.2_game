...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        File fileSave1 = new File("savegames/savePlayer1.dat");
        File fileSave2 = new File("savegames/savePlayer2.dat");
        File fileSave3 = new File("savegames/savePlayer3.dat");
        ...
        for (String path : paths) {
            zipFiles("/Users/daniil/Games/core_3.2_game/savegames/playersSave.zip", path);
        }
    ...
    public static String deleteFile(File file){
        if (file.delete()){
             return (String.format("Фай %s удалён \n", file.getName()));
        } else {
            return (String.format("Фай %s не получилось удалить\n", file.getName()));
        }
    }
}
